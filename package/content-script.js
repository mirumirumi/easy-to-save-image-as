...
function readContentSettings() {
    ...
    chrome.storage.sync.get((settingsReaded) => { //none 1st arg is all read
        ...
        for (let i = 0; i < 9; i++) {
            ...
            if (keys[i].includes(page[0])) { //save image as
                settings[0].push(settingsReaded[keys[i]]);
            }
            if (keys[i].includes(page[1])) { //copy image
                settings[1].push(settingsReaded[keys[i]]);
            }
            if (keys[i].includes(page[2])) { //copy image url
                settings[2].push(settingsReaded[keys[i]]);
            }
        }
        for (let i = 0; i < 3; i++) { //fill in "" for undefined
            if (settings[i].length === 0) {
                settings[i][0] = ""; settings[i].push(""); settings[i].push("");
            }
        }
    });
}
...
function execShortcut(mode, obj) {
    ...
    if (imgUrl.match(/https?:\/\//) !== null) { //case of full path -> sonomama
        //pass
    } else { //case of relative path
...
$(document).ready(function () {
    ...
    css.color = "#fff";
    css.fontWeight = "bold";
    css.fontFamily = "-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif";
