...
function fixKeyString(keyString) {
  return keyString.replace(/^ */gi, "").replace(/Control/gi, "Ctrl")
}
...
$("#save-btn").click(function saveSettings() {
  ...
  readSettingsKeys()
  setTimeout(() => {
    //storage.get is async func, better way -> promice.then()
    switch (page) {
...
function readSettingsKeys() {
  chrome.storage.sync.get(
    ["keys_/save-image-as.html", "keys_/copy-image.html", "keys_/copy-image-url.html"],
    (result) => {
